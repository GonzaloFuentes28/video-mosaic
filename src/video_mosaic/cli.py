...
from .extract import extract_all_frames, extract_every_n_seconds, extract_n_frames
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("input", help="Path to a video file or URL (YouTube, Twitter/X, TikTok, etc.).")
    parser.add_argument(
        "-o", "--output",
        default="mosaic.jpg",
    ...
    mode.add_argument("--all", action="store_true", help="Extract every frame (warning: can be huge).")
    mode.add_argument("--every", metavar="INTERVAL", help="One frame every INTERVAL (e.g. 5s, 500ms).")
    mode.add_argument("--frames", type=int, metavar="N", help="Exactly N evenly-spaced frames.")

    # Trim
    parser.add_argument("--from", dest="trim_start", metavar="TIME", help="Start time (e.g. 1:30, 90, 0:05.5).")
    ...
    parser.add_argument("--skip-dupes", action="store_true", help="Exclude near-duplicate consecutive frames.")

    # Layout options
    parser.add_argument("--cols", type=int, default=None, help="Number of columns (default: auto).")
    parser.add_argument("--thumb-width", type=int, default=None, help="Thumbnail width in px (default: native).")
    parser.add_argument("--padding", type=int, default=2, help="Padding between frames in px (default: 2).")
    parser.add_argument("--bg", default="#1a1a1a", help="Background color (default: #1a1a1a).")
    parser.add_argument("--labels", action="store_true", help="Show frame number + timestamp under each thumbnail.")
    parser.add_argument("--no-header", action="store_true", help="Don't render video info header on the mosaic.")
    parser.add_argument("--reverse", action="store_true", help="Reverse frame order (last frame first).")

    # PDF export
    parser.add_argument("--pdf", metavar="PATH", help="Also export as a multi-page PDF storyboard.")

    # URL options
    parser.add_argument("--quality", type=int, metavar="PX", help="Max video height for URL downloads (e.g. 720, 1080).")
...
def main() -> None:
    ...
    try:
        if is_url(args.input):
            ...
            if show_progress:
                progress = _make_progress(BarColumn(bar_width=30), TextColumn("[dim]{task.percentage:>5.1f}%[/dim]"))
                with progress:
                    ...
                    download_video(args.input, downloaded_path, quality=args.quality, on_progress=on_download)
                    progress.update(task, completed=100)
            else:
                download_video(args.input, downloaded_path, quality=args.quality)

            video_path = downloaded_path
        else:
            video_path = args.input
            if not os.path.isfile(video_path):
                _error(f"file not found → {video_path}")
            if args.quality:
                console.print("[warning]Warning:[/warning] --quality is only used with URL inputs, ignoring.")
        ...
        if trim_start is not None and trim_end is not None and trim_start >= trim_end:
            _error(f"--from ({fmt_timestamp(trim_start)}) must be before --to ({fmt_timestamp(trim_end)}).")
        if trim_start is not None and trim_start > duration:
            _error(f"--from ({fmt_timestamp(trim_start)}) is beyond video duration ({fmt_timestamp(duration)}).")
        if trim_end is not None and trim_end > duration:
            console.print(f"[warning]Warning:[/warning] --to exceeds duration, clamping to {fmt_timestamp(duration)}.")
            trim_end = duration
        ...
        try:
            if args.all:
                ...
                with progress:
                    task = progress.add_task("Extracting all frames …", total=1)
                    frames = extract_all_frames(video_path, tmp_dir, duration, trim_start, trim_end)
                    progress.update(task, completed=1, description=f"Extracted {len(frames)} frames")
            elif args.every:
                progress = _make_progress()
                with progress:
                    task = progress.add_task(f"Extracting frames (1 every {interval}s) …", total=1)
                    frames = extract_every_n_seconds(video_path, tmp_dir, interval, trim_start, trim_end)
                    progress.update(task, completed=1, description=f"Extracted {len(frames)} frames")
            else:
                n = args.frames
                if show_progress:
                    ...
                    with progress:
                        ...
                        frames = extract_n_frames(video_path, tmp_dir, n, duration, trim_start, trim_end, on_progress=on_extract)
                        progress.update(task, description=f"Extracted {len(frames)} frames")
                else:
                    frames = extract_n_frames(video_path, tmp_dir, n, duration, trim_start, trim_end)

            if not frames:
                _error("no frames were extracted. Check your trim range and input file.")
            ...
            if args.skip_black or args.skip_dupes:
                if show_progress:
                    ...
                    with progress:
                        ...
                        frames, removed = filter_frames(frames, args.skip_black, args.skip_dupes, on_progress=on_filter)
                        desc = f"Filtered {len(frames)} frames"
                        if removed:
                            desc += f" [dim](-{removed})[/dim]"
                        progress.update(task, description=desc)
                else:
                    frames, removed = filter_frames(frames, args.skip_black, args.skip_dupes)

            if not frames:
                _error("all frames were filtered out. Try relaxing --skip-black / --skip-dupes.")
            ...
            if est_pixels > MAX_CANVAS_PIXELS and args.thumb_width is None:
                # Scale down to fit within the pixel budget
                ...
                console.print(
                    ...
                    f"({n_frames} frames would exceed {MAX_CANVAS_PIXELS // 1_000_000}MP limit)"
                )
            ...
            progress = _make_progress()
            with progress:
                task = progress.add_task("Composing mosaic …", total=1)
                ...
                progress.update(
                    task, completed=1,
                    description=f"Saved [bold]{args.output}[/bold] [dim]({canvas_w}×{canvas_h})[/dim]",
                )
            ...
            if args.pdf:
                ...
                with progress:
                    ...
                    progress.update(task, completed=1, description=f"Saved [bold]{args.pdf}[/bold]")
